...
//  if(key=='`') picking=true; 
  ...
  if(key=='c') {showControl=!showControl;}
  if(key==']') {showQuads=!showQuads;}
  if(key=='h') {showCurve=!showCurve;}
  if(key=='\\') {showKeys=!showKeys;}
  if(key=='f') {showPath=!showPath;}
  if(key=='|') {showCorrectedKeys=!showCorrectedKeys;}
  if(key=='k') {inverseKinematics = !inverseKinematics;}
  ...
  if(key=='e') {showWalkerConstruction = !showWalkerConstruction;}
...
void displayFooter()  // Displays help text at the bottom
    ...
    scribeFooter(menu,0); 
    scribeFooter(user1,5);
    scribeFooter(user2,4);
    scribeFooter(user3,3);
    scribeFooter("Current subdvision level = " + level,2);
    }

String user1 = "display control polygon : c , display subdvided loop : h , increment/decrement subdivision level : -,=";
String user2 = "display foot curve/acceleration loop : f , Inverse Kinematics enable : k";
String user3 = "show improved walker : s , show telescopic walker / construction : e";
String title ="3D curve editor", name ="Lucas Liu",
