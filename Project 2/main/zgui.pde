void keyPressed() 
    ...
    if(key=='*') roomba.movementState = (roomba.movementState + 1) % 3;    
    if(key=='(') ;
    if(key==')') ;  
    ...
    if(key=='a') {      robotPause = !robotPause; }
    ...
    if(key=='j') {roomba.volume += 400000; println("roomba volume is", roomba.volume);}
    if(key=='k') {roomba.volume -= 400000; println("roomba volume is", roomba.volume);} 
    if(key=='l') {roomba.rays += 10; println("roomba ray count is", roomba.rays);} 
    if(key=='m') {roomba.rays -= 10; println("roomba ray count is", roomba.rays);} 
    if(key=='n') M.next();
    if(key=='o') M.opposite();  
    if(key=='p') M.previous();
    if(key=='q') {detectingPillars = !detectingPillars; roomba = new robot(M,R,0); }
    if(key=='r') roomba = new robot(M,R,0); 
    if(key=='s') M.swing();
    if(key=='t') ; 
    if(key=='u') M.unswing();
    if(key=='v') ; 
    ...
    if(key=='{') useColinearTest = ! useColinearTest;
    if(key=='}') useCenterOfMassMidpoint = !useCenterOfMassMidpoint;
    if(key=='|') ; 
    if(key=='[') ;
    if(key==']') ; 
    if(key=='\\') ;
